 ...
 */
public class GridT<T> implements Cloneable{

    private int sizex;
    private int sizey;
    private ArrayList<ArrayList<T>> grid;

    public  GridT(int sizex, int sizey) throws NegativeGridSizeException{
        this.sizex = sizex;
        this.sizey = sizey;
        try{
     ...
     */
    public void addPoint(int x, int y, T value) throws IndexOutOfBoundGridException {
        try {
     ...
     * @throws IndexOutOfBoundGridException  si x < 0 ou x > sizex-1 ou y < 0 ou y > sizey-1
     */
    public T getPoint(int x, int y) throws IndexOutOfBoundGridException {
        try {
    ...
    public boolean hasPoint(int x, int y) throws IndexOutOfBoundGridException {
        try {
            return this.grid.get(x).get(y) != null;
        }
        catch (IndexOutOfBoundsException e){
            throw new IndexOutOfBoundGridException(this.sizex, this.sizey, x, y);
        }
     ...
     */
    public GridT mergeLine(int line) throws IndexOutOfBoundGridException {
     ...
     */
    public GridT mergeLines(Integer... lines){
     ...
     * @throws UnmergeableColumnGridException si deux lignes de lines ne peuvent être fusionnées.
     */
    public GridT<T> mergeLines(List<Integer> lines)
            throws IndexOutOfBoundGridException, UnmergeableColumnGridException {
        try {
            if (lines.size() == 0)
                return (GridT<T>) (this.clone());
        }
        ...
        GridT<T> g = new GridT<T>(this.sizex-lines.size(), this.sizey);
        T elem;
        for(int x = 0; x<this.sizex; x++)
            for(int y = 0; y<this.sizey; y++){
                if((elem = this.getPoint(x,y)) != null) {
                    if (g.hasPoint(x - offsets[x], y))
                        throw new UnmergeableColumnGridException(lines, x, y);
                    else
                        g.addPoint(x - offsets[x], y, elem);
                }
            }
        return g;
    }
     ...
     */
    public GridT mergeColumn(int column) throws IndexOutOfBoundGridException, UnmergeableColumnGridException {
     ...
     */
    public GridT mergeColumns(Integer... columns){
     ...
     * @throws UnmergeableColumnGridException si deux colonnes de columns ne peuvent être fusionnées.
     */
    public GridT<T> mergeColumns(List<Integer> columns)
            throws IndexOutOfBoundGridException, UnmergeableColumnGridException {
        try {
            if (columns.size() == 0)
                return (GridT<T>) (this.clone());
        ...
        GridT<T> g = new GridT<T>(this.sizex, this.sizey-columns.size());
        T elem;
        for(int x = 0; x<this.sizex; x++)
            for(int y = 0; y<this.sizey; y++)
                if((elem = this.getPoint(x,y)) != null)
    ...
    public Object clone() throws CloneNotSupportedException {
        GridT<T> g = (GridT<T>)super.clone();
        g.grid = new ArrayList<ArrayList<T>>();
        for(int line = 0; line < getSizex(); line++) {
            ArrayList<T> ar = new ArrayList<T>();
            for (int column = 0; column < getSizey(); column++)
                ar.add(null);
    ...
    public Grid getGrid(){
        Grid g = new Grid(this.getSizex(), this.getSizey());
        for(int line = 0; line < this.getSizex(); line++)
            for(int column = 0; column < this.getSizey(); column++)
                if(this.hasPoint(line, column))
